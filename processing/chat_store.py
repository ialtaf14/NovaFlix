...
def get_conversation(user1: str, user2: str, limit: int = 50, offset: int = 0, is_group: bool = False) -> list:
    ...
    for r in reversed(rows):
        ...
        # Get reactions — shape: { emoji: [usernames] } (what the UI renders)
        c.execute("SELECT username, emoji FROM reactions WHERE message_id = ?", (msg['id'],))
        reactions = {}
        for row in c.fetchall():
            reactions.setdefault(row['emoji'], []).append(row['username'])
        msg['reactions'] = reactions

        # Seen status: for my own messages, has the other side seen it?
        if msg['sender'] == user1 and not is_group:
            c.execute("SELECT is_seen FROM message_status WHERE message_id = ? AND username = ?",
                      (msg['id'], user2))
            seen_row = c.fetchone()
            msg['seen'] = bool(seen_row['is_seen']) if seen_row else False
        else:
            msg['seen'] = True
        msgs.append(msg)
...
def add_reaction(user1: str, user2: str, msg_id: str, reactor: str, emoji: str, is_group: bool = False) -> dict:
    conn = get_db()
    c = conn.cursor()
    
    # Check existing reaction for this reactor on this message
    c.execute("SELECT emoji FROM reactions WHERE message_id = ? AND username = ?", (msg_id, reactor))
    row = c.fetchone()
    
    if row and row['emoji'] == emoji:
        # Tapping same emoji again removes the reaction (toggle off)
        c.execute("DELETE FROM reactions WHERE message_id = ? AND username = ?", (msg_id, reactor))
    elif emoji:
    ...
    # Return formatted reactions dict: { emoji: [username, ...] }
    c.execute("SELECT username, emoji FROM reactions WHERE message_id = ?", (msg_id,))
    reactions = {}
    for r in c.fetchall():
        reactions.setdefault(r['emoji'], []).append(r['username'])
    return reactions
...
def delete_message(user1: str, user2: str, msg_id: str, requester: str, for_everyone: bool = False, is_group: bool = False):
    ...
    if for_everyone:
        ...
        # Delete for me only (preserve other columns like is_seen/is_starred)
        c.execute("""
            INSERT INTO message_status (message_id, username, is_deleted)
            VALUES (?, ?, 1)
            ON CONFLICT(message_id, username) DO UPDATE SET is_deleted = 1
        """, (msg_id, requester))
    conn.commit()
...
def toggle_star_message(user1: str, user2: str, msg_id: str, requester: str, is_group: bool = False):
    ...
    c.execute("""
        INSERT INTO message_status (message_id, username, is_starred)
        VALUES (?, ?, ?)
        ON CONFLICT(message_id, username) DO UPDATE SET is_starred = excluded.is_starred
    """, (msg_id, requester, new_val))
    conn.commit()
...
def mark_read(msg_id: str, requester: str):
    ...
    c = conn.cursor()
    c.execute("""
        INSERT INTO message_status (message_id, username, is_seen)
        VALUES (?, ?, 1)
        ON CONFLICT(message_id, username) DO UPDATE SET is_seen = 1
    """, (msg_id, requester))
    conn.commit()

...
def get_mood_recommendations(username: str, mood: str, limit: int = 20, offset: int = 0):
    ...
    # Fetch real posters/year/rating concurrently (like smart-recommend does)
    from concurrent.futures import ThreadPoolExecutor
    titles_needed = [r["title"] for r in page]
    poster_map = {}
    def _fetch(t):
        poster, rating, year = preprocess.fetch_poster(t)
        return t, poster, rating, year
    with ThreadPoolExecutor(max_workers=min(20, max(len(titles_needed), 1))) as exc:
        for t, poster, rating, year in exc.map(_fetch, titles_needed):
            poster_map[t] = (poster, rating, year)

    res = []
    for r in page:
        t = r["title"]
        poster, rating, year = poster_map.get(t, (r["poster"], r["rating"], r["year"]))
        res.append({
            **r,
            "poster": poster,
            "rating": rating,

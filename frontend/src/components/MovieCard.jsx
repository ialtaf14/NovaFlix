...
function MovieCard({ title, poster, rating, year, provider, novaflix_rating }) {
  ...
  }, [isWishlisted, title, updateUser])

  const handleWatchedClick = useCallback(async (e) => {
    e.stopPropagation()
    try {
      if (isWatched) {
  ...
  }, [isWatched, title, watched, wishlist, updateUser])
  ...
  }, [isFavorite, title, updateUser])

  const handleRecommendClick = useCallback((e) => {
    e.stopPropagation()

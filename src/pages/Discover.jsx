    ...
    <div className="mood-movie-card" onClick={() => navigate(`/movie?title=${encodeURIComponent(title)}`)}>
      <div className="mood-movie-poster-wrapper">
        ...
        <div className="mood-movie-actions">
          <button
            className="mood-action-btn btn-recommend-quick"
...
function RowSection({ title, movies, loading }) {
...
export default function Discover() {
  ...
  const handleMoodClick = async (mood) => {
    ...
    setMoodMovies([])       // clear immediately so stale data doesn't show
    setMoodLoading(true)
    setMoodError('')
    setMoodOffset(0)
    setMoodHasMore(true)
    try {
      ...
      setMoodError('Could not load recommendations. Please try again.')
    }
    ...
    <div className="page fade-up">
      ...
      <div className="container">
            ...
            <div className="glass-panel mood-results-panel fade-up" style={{ marginTop: '1.5rem', padding: '1.5rem', borderRadius: '16px', background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255, 255, 255, 0.05)' }}>
              ...
              {moodLoading ? (
                ...
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '0.75rem', padding: '2rem 0' }}>
                  <span style={{ color: '#ff4b2b', fontSize: '0.9rem', fontWeight: 600 }}>⚠️ {moodError}</span>
                  <button
                    onClick={() => handleMoodClick(selectedMood)}
                    style={{ background: 'rgba(255,75,43,0.15)', border: '1px solid rgba(255,75,43,0.4)', color: '#ff4b2b', padding: '6px 18px', borderRadius: '8px', fontSize: '0.82rem', fontWeight: 700, cursor: 'pointer' }}
                  >
                    🔄 Try Again
                  </button>
                </div>
        ...
        {!trendingLoading && !latestLoading && !personalizedLoading &&
          ...
          <div style={{
            ...
            <h3 style={{ color: '#fff', marginBottom: '0.5rem' }}>Unable to Load Movies</h3>
      ...
      <style>{`
        ...
        .hero-slider {
          ...
          box-shadow: 0 20px 50px rgba(0, 0, 0, 0.35), inset 0 1px 1px rgba(255, 255, 255, 0.15);
        }
        ...
        .hero-slider-bg {
          ...
          filter: blur(20px) brightness(0.4) opacity(0.5);
        ...
        .hero-slider-overlay {
          ...
          background: linear-gradient(135deg, rgba(255, 255, 255, 0.05) 0%, rgba(0, 0, 0, 0.25) 100%),
                      linear-gradient(to right, rgba(12, 10, 25, 0.75) 0%, rgba(12, 10, 25, 0.35) 60%, transparent 100%),
                      linear-gradient(to top, rgba(12, 10, 25, 0.8) 0%, transparent 40%);
        }
        .hero-slider-content {
          position: relative;
          z-index: 2;

...
export default function Messages() {
  ...
  useEffect(() => {
    ...
    socket.on('message_reacted', ({ msg_id, reactions, emoji, username }) => {
      setMessages(prev => prev.map(m => {
        if (m.id === msg_id) {
          if (reactions) {
            return { ...m, reactions }
          ...
          const userList = updated[emoji] || []
          if (userList.includes(username)) {
            updated[emoji] = userList.filter(u => u !== username)
          } else {
            updated[emoji] = [...userList, username]
          }
  ...
  const fetchConversations = async (showSpinner = true) => {
    ...
    try {
      ...
      if (status === 401 || status === 403) {
        setConvError('Session expired. Please log in again.')
      } else if (!err.response) {
        setConvError('Could not reach the server. Make sure the backend is running.')
      } else {
        setConvError(`Server error (${status}). Please try again.`)
      }
    } finally {
  ...
  const handleSendMessage = (text = null, gifUrl = null, customType = null) => {
    const content = text || gifUrl || inputText
    if (!content.trim() || !activeConv) return

    const msgType = customType || (gifUrl ? "gif" : "text")
    const msgPayload = {
  ...
  const handleImageSelected = (e) => {
    ...
    reader.onload = (ev) => {
      handleSendMessage(ev.target.result, null, 'image')
    }
  ...
  const handleAddReaction = async (msgId, emoji) => {
    ...
    try {
      if (socketRef.current?.connected) {
        ...
        const { data } = await api.post(`/chat/${activeConv.username}/react/${msgId}`, { emoji })
        if (data?.reactions) {
          ...
          {activeConv ? (
            <>
              ...
              <div className="chat-footer-row">
                ...
                <button className="chat-emoji-btn" title="Add Photo" onClick={() => imageInputRef.current?.click()}>📷</button>
...
function MessageBubble({ msg, currentUser, otherAvatar, showAvatar, messages, isLastMine, onReact, onReply, onUnsend, onEdit, onDeleteForMe }) {
  ...
  const renderContent = () => {
    ...
    if (msg.type === 'image' || msg.content?.startsWith('data:image/') || (msg.content?.startsWith('http') && (msg.content?.endsWith('.jpg') || msg.content?.endsWith('.png') || msg.content?.endsWith('.jpeg') || msg.content?.endsWith('.webp')))) {
      return (
        <div className="chat-shared-image-container">
          <img src={msg.content} alt="shared media" className="chat-shared-image" onClick={() => window.open(msg.content, '_blank')} />
        </div>
      )
    }

    if (msg.type === 'audio' || msg.content?.startsWith('data:audio/')) {
      return (
        <div className="chat-shared-audio-container">
          <audio controls src={msg.content} className="chat-audio-player" />
        </div>
      )
    }
